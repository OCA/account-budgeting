...
class TestBudgetControl(TransactionCase):
    ...
    def test_01_no_budget_control_sheet(self):
        """As budget mgnt was created to control costcenter1 and costcenter2
        yet no budget control sheet has been created. Lock budget immediatlly.
        """
    ...
    def test_02_budget_control_amount_exceed(self):
        ...
        ctx = {'active_id': budget_mgnt.id,
               'default_analytic_group_ids': [self.group_costcenter.id],
        ...
        # According to 3 KPIs x 4 quarter, this result in 12 budget items
        self.assertEqual(len(budget_ctrl.item_ids), 12)
        # Allocate budget 1,000 for each period and each kpi, totaling 12,000
        # Each KPI, will has amount = 4000
    ...
    def test_04_budget_carry_over(self):
        ...
        On previous year for costcenter1
            - Total Budget on 3 KPIs = 12000 (1 KPI = 4 quarter x 1000 = 4000)
            - Bill Actual on 1 KPI = 3000
            - PO commit = 1000
            - Budget Balance = 12000-3000-1000 = 8500
        Carry over to new year, only commitment will carry over,
            (Commitment of previous year will be 0)
            - PO Commit Carry Forward = 1000 on new year
            - Budget Carry Over = 1000, equal to commitment
            - Note that, initially all budget balance will be 0.0
        """
        # To simpulate, first create budget of last year.
        prev_budget_mgnt = self.create_budget_management(self.prev_year,
                                                         [self.costcenter1.id])
        view_id = "budget_control.view_generate_budget_control"
        ctx = {'active_id': prev_budget_mgnt.id,
               'default_analytic_group_ids': [self.group_costcenter.id],
               'default_all_analytic_accounts': True}
        f = Form(self.wiz_gen_budget_ctrl.with_context(ctx), view=view_id)
        wizard = f.save()
        ...
        invoice = self.create_invoice(self.costcenter1, 3000.0, prev_date)
        invoice.action_invoice_open()
        # Commit on previous year on purchases, 400 + 600 = 1000
        po1 = self.create_purchase_order(self.costcenter1, 400.0, prev_date)
        po1.with_context({'commit_by_docdate': True}).button_confirm()
        po2 = self.create_purchase_order(self.costcenter1, 600.0, prev_date)
        ...
        self.assertEqual(commit, 1000.0)
        self.assertEqual(year_forward, self.this_year)
        # Do budget carry forward!
        ...
        ctx = {'active_id': this_budget_mgnt.id,
               'default_analytic_group_ids': [self.group_costcenter.id],
               'default_all_analytic_accounts': True,
               'default_init_budget_commit': True,
               }
        f = Form(self.wiz_gen_budget_ctrl.with_context(ctx), view=view_id)
        wizard = f.save()
        wizard.action_generate_budget_control()
        budget_ctrl = self.obj_budget_ctrl.search([
            ...
            ('analytic_account_id', '=', self.costcenter1.id)])
        # New inital commit is 1000, equal to the carry forward amount
        self.assertEquals(sum(budget_ctrl.item_ids.mapped('amount')), 1000.0)

...
class BudgetLine(orm.Model):
    ...
    def _prac_amt(self, cr, uid, ids, context=None):
        ...
        budget_lines = self.browse(cr, uid, ids, context=context)
        results = {}
        ids_with_accounts = []
        for line in budget_lines:
            # Normally the standard reacts to account-less budget position with
            # an error, but they do make sense if you want to count only POs, so
            # we'll count them as 0 lines instead
            if not line.general_budget_id.account_ids:
                results[line.id] = 0
            else:
                ids_with_accounts.append(line.id)
        results.update(super(BudgetLine, self)._prac_amt(
            cr, uid, ids_with_accounts, context=context))
